...
import { NewPatient, Gender, Entry, TypedBaseEntry, HospitalEntry, OccupationalHealthcareEntry, HealthCheckEntry, HealthCheckRating } from "./types";
...
const isEntry = (data: any): data is TypedBaseEntry => {
  ...
  if (!description || !isString(description)) {
    throw new Error('Incorrect or missing entry description: ' + JSON.stringify(data));
  }
  if (!specialist || !isString(specialist)) {
    throw new Error('Incorrect or missing entry specialist: ' + JSON.stringify(data));
  }
...
const isOccupationalHealthcareEntry = (data: any): data is OccupationalHealthcareEntry => {
  ...
  if (!employerName || !isString(employerName)) {
    throw new Error('Incorrect OccupationalHealthcareEntry: ' + JSON.stringify(data));
  }
...
type Fields = { name :unknown, gender: unknown, dateOfBirth: unknown,occupation: unknown, ssn: unknown, entries?: unknown };

export const toNewPatient = ({ name, gender, dateOfBirth, occupation, ssn, entries } : Fields): NewPatient => {
  return {
